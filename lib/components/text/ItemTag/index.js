...
export default function Index(props) {
  ...
  const bg = solid ? `${color}` : outline && plain ? '#ffffff' : outline && none || outline && dash || outline ? `${color}99` : null;
  const border = dash && outline ? `2px ${color} dashed` : outline && none ? null : outline && plain || outline ? `2px ${color} solid` : null;
  const colors = solid ? '#ffffff' : `${color}`; //   const Radius = circle && size  ? '50px': 0
  ...
  const baseStyle = {
    ...
    border: `${border}`,
    minWidth: `${minWidth}`,
    borderRadius: `${Radius}` // width:`${circleWidth}`,
    // height:`${circleHeight}`
